...
def main(path, dylib):
    ...
    for _ in range(ncmds):
        ...
        if cmd in (0xC, 0x80000018, 0x8000001F, 0x80000020, 0x80000023):
            ...
            if end > start and data[start:end].decode("utf-8", "ignore") == dylib:
                print("Already injected:", dylib)
                return
        off += cmdsize
    ...
    struct.pack_into("<IIIIII", command, 0, LC_LOAD_DYLIB, cmdsize, 24, 2, 0x10000, 0x10000)
